...
def test_get_items() -> None:
    ...
    item_name = json.loads(response.json()[0])["name"]
...
def test_get_item() -> None:
    ...
    item_id = json.loads(response.json()[1])["id"]
    response = client.get(f"/item/{item_id}")

    assert response.status_code == 200
    assert json.loads(response.json())["name"] == "Test Get Item"
...
def test_patch_item() -> None:
    ...
    assert json.loads(response.json())["id"] == item_id

    assert response.status_code == 200
    assert json.loads(response.json())["name"] == "Gary Item"
