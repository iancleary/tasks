...
class FastAPISessionMaker:
    ...
    https://github.com/yuval9313/FastApi-RESTful/blob/19655d19c78d95de15e45b71c002e3324c87a099
        /fastapi_restful/session.py#L1
        /LICENSE (MIT)
    """
    ...
    def __init__(self, database_uri: str):
        ...
        In particular, `sqlalchemy.create_engine(database_uri)`
        should work to create an engine.
    ...
    def cached_sessionmaker(self) -> sa.orm.sessionmaker:
        """
        Returns a lazily-cached sqlalchemy sessionmaker
        using the instance's (lazily-cached) engine.
        """
    ...
    def get_db(self) -> Iterator[Session]:
        """
        A generator function that yields a sqlalchemy orm session
        and cleans up the session once resumed after yielding.

        Can be used directly as a context-manager FastAPI dependency,
        or yielded from inside a separate dependency.
        """
    ...
    def context_session(self) -> Iterator[Session]:
        ...
        This makes it possible to get a context-managed orm session
        for the relevant database_uri without needing to rely on FastAPI's
    ...
    def reset_cache(self) -> None:
        ...
        After calling this method, the next time you try to use the
        cached engine or sessionmaker, new ones will be created.
        """
...
def get_engine(uri: str) -> sa.engine.Engine:
    ...
    This function may be updated over time to reflect recommended engine
    configuration for use with FastAPI.
    """
...
def get_sessionmaker_for_engine(engine: sa.engine.Engine) -> sa.orm.sessionmaker:
    """
    Returns a sqlalchemy sessionmaker for the provided engine with
    recommended configuration settings.

    This function may be updated over time to reflect recommended
    sessionmaker configuration for use with FastAPI.
    """
...
def context_session(engine: sa.engine.Engine) -> Iterator[Session]:
    ...
    Usage is similar to `FastAPISessionMaker.context_session`,
    except that you have to provide the engine to use.

    A new sessionmaker is created for each call,
    so the FastAPISessionMaker.context_session method may be
    preferable in performance-sensitive contexts.
    """
...
def _get_db(sessionmaker: sa.orm.sessionmaker) -> Iterator[Session]:
    """
    A generator function that yields an ORM session using the provided
    sessionmaker, and cleans it up when resumed.
    """
